...
const DEFAULT_COUNT_THRESHOLDS = {
  ...
  front_occupied_km2: 10,  // ±10 km² of DeepState-assessed occupied Ukraine
  cartel_wars: 1,          // any inter-cartel war added to / removed from the crowd-sourced map
  iran_events_48h: 5,      // ±5 LLM-extracted Iran-theater events in the 48 h window
};
...
const COUNT_METRICS = [
  ...
  { key: 'sources_ok', extract: d => d.meta?.health?.live ?? d.meta?.sourcesOk ?? 0, label: 'Sources Live' },
  // NaN when the feed is not live so an outage never reads as territory changing hands
  { key: 'front_occupied_km2', extract: d => (d.frontlines?.status === 'live' && d.frontlines.occupiedKm2 > 0) ? d.frontlines.occupiedKm2 : NaN, label: 'Occupied Ukraine (km², DeepState)' },
  // NaN unless the KML was fetched fresh this sweep, so a cached copy or outage never reads as a change
  { key: 'cartel_wars', extract: d => (d.cartels?.status === 'live' && !d.cartels.stale && Array.isArray(d.cartels.wars)) ? d.cartels.wars.length : NaN, label: 'Active cartel wars mapped (MX)' },
  // NaN unless the feed was live this sweep, so an outage or cached copy never reads as the tempo changing
  { key: 'iran_events_48h', extract: d => (d.iranwar?.status === 'live' && !d.iranwar.stale) ? (d.iranwar.counts?.events48h ?? NaN) : NaN, label: 'Iran-theater events / 48 h (IranWarLive)' },
  // NaN unless the snapshot was served (fresh or cached — TRAC re-publishes only every few weeks)
  { key: 'ice_detained', extract: d => (['live', 'partial', 'stale'].includes(d.trac?.status) ? (d.trac.population?.latest?.total ?? NaN) : NaN), label: 'ICE detained population (TRAC)' },
  // NaN unless MND's latest daily report parsed, so a blocked or unreachable site never reads as calm
  { key: 'pla_sorties_daily', extract: d => (['live', 'partial', 'stale'].includes(d.plamnd?.status) ? (d.plamnd.activity?.latest?.aircraft ?? NaN) : NaN), label: 'PLA aircraft sorties around Taiwan / day (MND)' },
];
...
export function computeDelta(current, previous, thresholdOverrides = {}, priorRuns = []) {
  ...
  // ─── ACLED FLASH alerts (fatalities > 50 in a single event) ─────────

  const acledFlash = current.acled?.flashAlerts || [];
  for (const alert of acledFlash) {
    signals.new.push({
      key: `acled_flash:${alert.headline}`,
  ...
  // ─── ADS-B PRIORITY alerts (military over sensitive regions) ───────

  const adsbAlerts = current.adsbMilitary?.priorityAlerts || [];
  for (const alert of adsbAlerts.slice(0, 5)) {
    signals.new.push({
      key: `adsb_priority:${alert.headline}`,
      reason: alert.headline,
      detail: alert.detail,
      severity: 'high',
      tier: 'PRIORITY',
  ...
  // ─── GDELT tone deterioration PRIORITY alerts ─────────────────────

  const gdeltAlerts = current.gdelt?.priorityAlerts || [];
  for (const alert of gdeltAlerts) {
    signals.new.push({
      key: `gdelt_tone:${alert.headline}`,
      reason: alert.headline,
      detail: alert.detail,
      severity: 'high',
      tier: 'PRIORITY',
  ...
  // ─── InSight Crime sanctions cross-ref PRIORITY alerts ────────────

  const icAlerts = current.insightCrime?.priorityAlerts || [];
  for (const alert of icAlerts) {
    signals.new.push({
      key: `ic_sanctions:${alert.headline}`,
      reason: alert.headline,
      detail: alert.detail,
      severity: 'high',
      tier: 'PRIORITY',
  ...
  // ─── Unusual Whales PRIORITY alerts (options flow >$5M) ─────────────

  const uwAlerts = current.unusualWhales?.priorityAlerts || [];
  for (const alert of uwAlerts) {
    signals.new.push({
      key: `uw_options:${alert.ticker}`,
      reason: alert.headline,
      detail: alert.detail,
      severity: 'high',
      tier: 'PRIORITY',
  ...
  // ─── Unusual Whales FLASH alerts (defense committee + geopolitical) ──

  const defenseCommitteeTrades = current.unusualWhales?.congressTrades?.defenseCommittee || [];
  const geopoliticalAlerts = [
    ...(current.gdelt?.priorityAlerts || []),
    ...(current.acled?.flashAlerts || []),
    ...(current.adsbMilitary?.priorityAlerts || []),
  ];

  if (geopoliticalAlerts.length > 0) {
    const now = new Date();
    const last48h = new Date(now.getTime() - 48 * 60 * 60 * 1000);

    for (const trade of defenseCommitteeTrades) {
      const tradeDate = new Date(trade.transactionDate);
      if (tradeDate >= last48h) {
        signals.new.push({
          key: `uw_flash:${trade.name}:${trade.ticker}`,
          reason: `DEFENSE COMMITTEE TRADE: ${trade.name} traded ${trade.ticker} within 48h of geopolitical alert`,
          detail: `${trade.txnType} ${trade.amounts} on ${trade.transactionDate}`,
          severity: 'critical',
          tier: 'FLASH',
        });
        criticalChanges += 3;
