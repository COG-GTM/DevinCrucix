...
// Globe textures are referenced protocol-relative (//unpkg.com/...), which resolves to http: when the
// dashboard itself is served over plain http (local dev). Over https the generic `https:` source applies.
const IMG_HOSTS = ['http://unpkg.com'];

export const CONTENT_SECURITY_POLICY = [
  "default-src 'self'",
  `script-src 'self' 'unsafe-inline' ${SCRIPT_HOSTS.join(' ')}`,
  `style-src 'self' 'unsafe-inline' ${STYLE_HOSTS.join(' ')}`,
  `img-src 'self' data: blob: https: ${IMG_HOSTS.join(' ')}`,
