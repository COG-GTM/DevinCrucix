...
import { evaluateLayers } from './maplayers.mjs';

export const SEVERITIES = ['critical', 'high', 'elevated', 'info'];
export const TABS = ['situation', 'military', 'cyber', 'macro', 'ukraine', 'cartels', 'iranwar', 'taiwan', 'investigations', 'requirements', 'sources'];
...
const RULES = [
  ...
  function borderSpikes(V2) {
    ...
    return [headline('border', 'high',
      ...
      'Border Watch', 'cartels', 'border-panel')];
  },

  function front(V2) {
    const f = V2.frontlines || {};
    const h = f.history || {};
    if (f.status !== 'live' || !num(h.recent7d)) return [];
    const latest = list(h.updates)[0];
    const adv = num(h.advances7d), reg = num(h.regains7d);
    const sev = adv >= 5 && adv > reg * 2 ? 'elevated' : 'info';
    const moved = list(V2.delta?.signals?.escalated).concat(list(V2.delta?.signals?.deescalated))
      .find(s => s.key === 'front_occupied_km2');
    const movedTxt = moved ? ` Occupied area ${moved.change > 0 ? '+' : ''}${fmtNum(moved.change)} km² vs last sweep.` : '';
    return [headline('front', sev,
      `Ukraine front: ${h.recent7d} DeepState update${h.recent7d === 1 ? '' : 's'} in 7d · ${adv} advance${adv === 1 ? '' : 's'}, ${reg} regain${reg === 1 ? '' : 's'}`,
      `${latest?.text ? str(latest.text, 120) + ' ' : ''}~${fmtNum(f.occupiedKm2)} km² assessed occupied (DeepStateMAP assessment, not verified ground truth).${movedTxt}`,
      'DeepStateMAP', 'ukraine', 'front-panel')];
  },

  // Current KML observations only (never the 2020 baseline), and only when the map itself is fresh:
  // a crowd-sourced source earns an `info` pointer to its evidence, not an alert.
  function cartels(V2) {
    const c = V2.cartels || {};
    if (c.status !== 'live' || c.stale || !num(c.activityLast30d) || !c.latestMention) return [];
    const ageD = (Date.now() - Date.parse(c.latestMention)) / 86400000;
    if (!Number.isFinite(ageD) || ageD < 0 || ageD > 7) return [];
    const wars = list(c.wars).length;
    return [headline('cartels', 'info',
      `Mexico cartel map: ${c.activityLast30d} dated entr${c.activityLast30d === 1 ? 'y' : 'ies'} in 30d${wars ? ` · ${wars} active wars mapped` : ''}`,
      `Latest dated entry ${str(c.latestMention, 10)}; ${num(c.orgCount)} organizations across ${num(c.layerCounts?.influence)} influence areas. Crowd-sourced single-maintainer map (@MexicoCartelMap) — unverified leads, not control-of-territory truth.`,
      'Cartel map (KML)', 'cartels', 'cartel-map-panel')];
  },

  // LLM-extracted wire aggregator: an `info` pointer to the evidence when the feed is fresh and busy,
  // never an alert on its own (single wire, city-level coordinates, no human review).
  function iranwar(V2) {
    const w = V2.iranwar || {};
    const c = w.counts || {};
    if (w.status !== 'live' || w.stale || !num(c.events24h)) return [];
    if (!Number.isFinite(w.feedAgeH) || w.feedAgeH > 6) return [];
    const k = c.byKind48h || {};
    const bits = [];
    if (num(k.missile)) bits.push(`${k.missile} missile`);
    if (num(k.drone)) bits.push(`${k.drone} drone`);
    if (num(k.air)) bits.push(`${k.air} air-strike`);
    if (num(c.ground48h)) bits.push(`${c.ground48h} ground-ops`);
    return [headline('iranwar', 'info',
      `Iran theater: ${c.events24h} extracted event${c.events24h === 1 ? '' : 's'} in 24 h · ${num(c.casualties48h)} reported casualties in 48 h`,
      `${bits.length ? bits.join(' · ') + ' rows in 48 h. ' : ''}Machine-extracted from English news wires by IranWarLive (single operator, no human review) — observational pointers, not verified events.`,
      'IranWarLive', 'iranwar', 'iwl-map-panel')];
  },

  // Official Taiwan MND daily bulletin: a pointer when the latest 24 h count is well above the 30-day
  // average, or when the Coast Guard logged a CCG intrusion in the last 48 h. Counts are aggregates
  // published by a government, not tracks; the severity stays `elevated` at most.
  function taiwan(V2) {
    const t = V2.taiwan || {};
    const b = t.mnd?.bulletin;
    const out = [];
    if (b && t.mnd?.status !== 'unavailable' && Number.isFinite(b.aircraft)) {
      const avg = t.mnd?.stats?.avgAircraft;
      const spike = Number.isFinite(avg) && avg > 0 && b.aircraft >= Math.max(20, avg * 2);
      if (spike) {
        out.push(headline('taiwan-mnd', 'elevated',
          `PLA activity around Taiwan: ${b.aircraft} aircraft sorties in MND's ${b.publishedDate} bulletin (30-day avg ${avg})`,
          `${num(b.adizEntries)} entered the ADIZ${b.sectors?.length ? ' (' + b.sectors.join(', ') + ')' : ''} · ${num(b.planShips)} PLAN ships · ${num(b.officialShips)} official ships. Official daily count from Taiwan's Ministry of National Defense.`,
          'Taiwan MND', 'taiwan', 'tw-mnd-panel'));
      }
    }
    const inc = (Array.isArray(t.cga?.incidents) ? t.cga.incidents : []).filter(i => i.kind === 'ccg-intrusion' && i.published && Date.now() - Date.parse(i.published) <= 48 * 3600000);
    if (inc.length && t.cga?.status !== 'unavailable') {
      const areas = [...new Set(inc.map(i => i.area?.en).filter(Boolean))].slice(0, 3).join(', ');
      const hulls = [...new Set(inc.flatMap(i => i.hulls || []))].slice(0, 6).join(', ');
      out.push(headline('taiwan-cga', 'info',
        `China Coast Guard intrusion${inc.length > 1 ? 's' : ''} reported by Taiwan CGA: ${inc.length} in 48 h${areas ? ' · ' + areas : ''}`,
        `${hulls ? 'Hulls ' + hulls + '. ' : ''}From Coast Guard Administration press releases (restricted / prohibited waters). Area-level only; no vessel positions.`,
        'Taiwan CGA', 'taiwan', 'tw-cga-panel'));
    }
    return out;
  },
  ...
  function cyberfix(V2) {
    const cf = V2.cyberfix || {};
    const confirmed = list(cf.exposures).filter(e => e?.confidence === 'confirmed');
    if (!confirmed.length) return [];
    const fresh = list(cf.newExposureKeys);
    const newConfirmed = confirmed.filter(e => fresh.includes(e.exposureKey));
    const lead = newConfirmed[0] || confirmed[0];
    const ransomware = confirmed.some(e => String(e.kev?.knownRansomwareCampaignUse).toLowerCase() === 'known');
    const remediating = list(cf.remediations).filter(r => confirmed.some(e => e.exposureKey === r.exposureKey) && r.status !== 'failed').length;
    const n = confirmed.length;
    return [headline('cyberfix', newConfirmed.length ? (ransomware ? 'critical' : 'high') : 'elevated',
      `${n} confirmed KEV exposure${n === 1 ? '' : 's'} in inventory${newConfirmed.length ? ` · ${newConfirmed.length} new` : ''}`,
      `${lead.cveId} in ${lead.component?.name}${lead.component?.version ? '@' + lead.component.version : ''}${lead.fixedVersion ? ` → fix ${lead.fixedVersion}` : ''}` +
        `${lead.kev?.dueDate ? ` · KEV due ${lead.kev.dueDate}` : ''}${remediating ? ` · ${remediating} remediation session${remediating === 1 ? '' : 's'} tracked` : ''}.`,
      'CISA KEV × inventory', 'cyber', 'cyberfix-panel')];
  },
