...
const stated = (v) => (Number.isFinite(v) ? v : '—');
    ...
    if (b && t.mnd?.status !== 'unavailable' && Number.isFinite(b.aircraft)) {
      ...
      if (spike) {
        out.push(headline('taiwan-mnd', 'elevated',
          ...
          `${stated(b.adizEntries)} entered the ADIZ${b.sectors?.length ? ' (' + b.sectors.join(', ') + ')' : ''} · ${stated(b.planShips)} PLAN ships · ${stated(b.officialShips)} official ships. Official daily count from Taiwan's Ministry of National Defense.`,
