...
const DEGRADED_STATUS = new Set(['limited', 'partial', 'fallback', 'web_scrape', 'stale', 'cached', 'empty', 'blocked']);
const ERROR_STATUS = new Set(['error', 'failed']);
const DERIVED_STATUS = new Set(['deferred', 'ready']);
const META_KEYS = new Set(['source', 'timestamp', 'status', 'message', 'note', 'hint', 'error', 'durationMs', 'method', 'dataSource', 'description', 'coverage', 'openskyError']);

// Sources that only work with a locally running sidecar process/container. When they are
// not reporting the dashboard hides their panels instead of showing an equal-weight "OFF" card.
export const SIDECAR_SOURCES = {
  PizzaIndex: 'needs pizza-index sidecar',
  SpiderFoot: 'needs SpiderFoot sidecar',
};
...
export function classifySource(name, src, timing, errorEntry) {
  const h = classifyInner(name, src, timing, errorEntry);
  if (SIDECAR_SOURCES[name]) {
    h.sidecar = true;
    if (h.state === 'off' || h.state === 'error') h.reason = SIDECAR_SOURCES[name];
  }
  return h;
}

function classifyInner(name, src, timing, errorEntry) {
  ...
  if (src.error) {
    if (envVars.length) return { ...base, state: 'no_key', reason: 'needs credentials', envVars };
    if (!dataKeys.length || ERROR_STATUS.has(status)) return { ...base, state: 'error', reason: shortError(src.error) };
    return { ...base, state: 'degraded', reason: shortError(src.error) };
  }
  if (status && ERROR_STATUS.has(status)) return { ...base, state: 'error', reason: shortError(src.message || status) };
