// China / Taiwan dashboard view model: the bounded slice of the six Taiwan adapters (MND bulletin +
// Skyfaring trend, CGA grey-zone incidents, Focus Taiwan / Taipei Times headlines, GCA observational
// strip, Polymarket threat markets, Iron Command derived context) that goes into the client payload,
// plus the existing CRUCIX signals for the theater (PRC tension composite, Taiwan Strait / SCS air,
// Chinese ISR, carriers).
...
const MAX_GCA = 24;
const MAX_MARKETS = 8;
const MAX_IC_EVENTS = 24;
const MAX_IC_SPARK = 14;
...
export const LINK_OUTS = [
  ...
  { key: 'ic-plan', name: 'Iron Command — PLA Navy Tracker', url: 'https://www.ironcommand.co/watch/plan', reason: 'OSINT inference of PLAN deployment posture, not a position feed; no machine-readable dataset behind it.' },
];
...
const IC_STATES = new Set(['SPIKE', 'ELEVATED', 'NORMAL', 'QUIET', 'UNKNOWN']);
const IC_GRADES = new Set(['A', 'B', 'C', 'D', 'E', 'F']);
export function trimIronCommand(i = {}) {
  return {
    source: 'IronCommand',
    status: health(i),
    stale: Boolean(i.stale),
    error: i.error ? str(i.error, 200) : null,
    problems: strList(i.problems, 6, 160),
    provider: str(i.provider, 60),
    siteUrl: httpUrl(i.siteUrl),
    methodologyUrl: httpUrl(i.methodologyUrl),
    planTrackerUrl: httpUrl(i.planTrackerUrl),
    license: str(i.license, 40),
    attribution: str(i.attribution, 120),
    fetchedAt: iso(i.fetchedAt),
    cacheAgeH: numOrNull(i.cacheAgeH),
    generatedAt: iso(i.generatedAt),
    generatedAgeH: numOrNull(i.generatedAgeH),
    embargoHours: numOrNull(i.embargoHours),
    windowHours: numOrNull(i.windowHours),
    feedEvents: num(i.feedEvents),
    chokepoints: (Array.isArray(i.chokepoints) ? i.chokepoints : []).slice(0, 4).map(c => ({
      key: str(c.key, 24), name: str(c.name, 40), lat: coord(c.lat, 90), lon: coord(c.lon, 180),
      events7d: num(c.events7d), eventsPrior7d: num(c.eventsPrior7d),
      trend: ['up', 'down', 'flat'].includes(c.trend) ? c.trend : 'flat',
      state: IC_STATES.has(c.state) ? c.state : 'UNKNOWN',
      spark: (Array.isArray(c.spark) ? c.spark : []).slice(-MAX_IC_SPARK).map(num),
      basis: str(c.basis, 24),
    })),
    tripwires: (Array.isArray(i.tripwires) ? i.tripwires : []).slice(0, 8).map(w => ({ description: str(w.description, 120), lastFired: date(w.lastFired), fired7d: Boolean(w.fired7d), theater: Boolean(w.theater) })),
    events: (Array.isArray(i.events) ? i.events : []).slice(0, MAX_IC_EVENTS).map(e => ({
      id: Number.isInteger(e.id) ? e.id : null,
      title: str(e.title, 200),
      place: e.place ? str(e.place, 60) : null,
      lat: coord(e.lat, 90),
      lon: coord(e.lon, 180),
      locationPrecision: 'centroid',
      grade: IC_GRADES.has(e.grade) ? e.grade : null,
      corroboration: num(e.corroboration),
      reports: num(e.reports),
      sourceCount: num(e.sourceCount),
      sources: strList(e.sources, 12, 60),
      disciplines: strList(e.disciplines, 6, 12),
      region: e.region ? str(e.region, 24) : null,
      first: iso(e.first),
      last: iso(e.last),
      url: httpUrl(e.url),
    })),
    stats: {
      shown: num(i.stats?.shown), theaterEvents: num(i.stats?.theaterEvents), mndDuplicates: num(i.stats?.mndDuplicates), offTopic: num(i.stats?.offTopic),
      byGrade: counts(i.stats?.byGrade, 7), spike: num(i.stats?.spike), elevated: num(i.stats?.elevated), tripwiresFired7d: num(i.stats?.tripwiresFired7d),
      feeds: num(i.stats?.feeds), items7d: num(i.stats?.items7d),
    },
    disclaimer: strList(i.disclaimer, 6, 240),
  };
}
...
export function buildTaiwanView(sources = {}, V2 = {}) {
  ...
  const ironcommand = trimIronCommand(sources.IronCommand || {});
  const parts = { mnd: mnd.status, cga: cga.status, news: news.status, gca: gca.status, markets: markets.status, ironcommand: ironcommand.status };
...
export function buildTaiwanGeo(sources = {}) {
  ...
  const ic = sources.IronCommand || {};
  for (const c of (Array.isArray(ic.chokepoints) ? ic.chokepoints : []).slice(0, 4)) {
    if (!Number.isFinite(c.lat) || !Number.isFinite(c.lon)) continue;
    features.push({
      type: 'ic-chokepoint', precision: 'reference', key: str(c.key, 24), name: str(c.name, 40), lat: c.lat, lon: c.lon,
      state: IC_STATES.has(c.state) ? c.state : 'UNKNOWN', trend: ['up', 'down', 'flat'].includes(c.trend) ? c.trend : 'flat',
      events7d: num(c.events7d), eventsPrior7d: num(c.eventsPrior7d), basis: str(c.basis, 24), generatedAt: iso(ic.generatedAt),
    });
  }
  return { generatedAt: new Date().toISOString(), count: features.length, features, attribution: gca.attribution ? str(gca.attribution, 120) : null, icAttribution: ic.attribution ? str(ic.attribution, 120) : null };
}
