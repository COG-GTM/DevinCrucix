...
export const LAYERS = [
  ...
  {
    // Iron Command chokepoint states (Taiwan Strait / Miyako / Scarborough): one badge per chokepoint at
    // its fixed reference point, coloured by the fused-event-count state. Derived, delayed, never `signal`.
    id: 'ic-chokepoints', label: 'Chokepoints · Iron Command', color: '#ffd54f', rank: 9.88,
    types: ['ic-chokepoint'], sources: ['IronCommand'],
    eval(V2) {
      const i = V2.taiwan?.ironcommand || {};
      const cps = Array.isArray(i.chokepoints) ? i.chokepoints : [];
      if (i.status === 'unavailable' || !cps.length) return { count: 0, signal: false, why: '' };
      const bits = [cps.map(c => `${c.name || c.key} ${c.state} (${num(c.events7d)} vs ${num(c.eventsPrior7d)} prior 7 d)`).join(' · '), 'event-count basis, 6 h delayed'];
      if (i.stale) bits.push('cached copy');
      return { count: cps.length, signal: false, why: bits.join(' · ') };
    },
  },
  {
