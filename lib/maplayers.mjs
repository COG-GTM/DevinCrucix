...
export const LAYERS = [
  ...
  {
    // State-centroid aggregates of TRAC's facility table, never individual facility positions.
    id: 'detention', label: 'ICE Detention (TRAC)', color: '#0294de', rank: 10.5,
    types: ['ice-detention'], sources: ['TRAC'],
    eval(V2) {
      const f = V2.trac?.facilities || {};
      const states = geo(f.byState).filter(s => num(s.detainees) > 0);
      const total = num(f.total);
      return { count: states.length, signal: false, why: states.length ? `${total.toLocaleString('en-US')} detained · ${num(f.facilities)} facilities` : '' };
    },
  },
  {
