...
export const LAYERS = [
  ...
  {
    id: 'air', label: 'Air Activity', color: '#64f0c8', rank: 3.8,
    types: ['air'], arcs: true, sources: ['OpenSky', 'ADS-B'],
    eval(V2) {
      const air = list(V2.air);
      const total = air.reduce((s, a) => s + num(a.total), 0);
      const tracks = air.reduce((s, a) => s + geo(a.tracks).length, 0);
      const mil = air.filter(a => num(a.military) > 0 || list(a.tracks).some(t => t?.mil === true));
      const emergency = air.filter(a => list(a.tracks).some(t => EMERGENCY_SQUAWKS.has(String(t?.squawk || ''))));
      const dark = air.filter(a => num(a.total) >= 20 && num(a.noCallsign) / a.total >= 0.15);
      const why = emergency.length ? `${emergency.map(a => a.region).join(', ')}: emergency squawk`
        : mil.length ? `${mil.map(a => a.region).join(', ')}: military aircraft on ADS-B`
          : dark.length ? `${dark.map(a => a.region).join(', ')}: ≥15% no callsign`
            : total ? `${air.filter(a => num(a.total) > 0).length} theaters · ${tracks} tracks plotted` : '';
      return { count: total, signal: emergency.length + mil.length + dark.length > 0, why };
    },
  },
  {
