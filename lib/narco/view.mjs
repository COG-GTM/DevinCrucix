...
function weight(kg) {
  ...
  if (kg >= 1) return `${Math.round(kg * 10) / 10} kg`;
  return `${Math.max(1, Math.round(kg * 1000))} g`;
}
function seizureSummary(sz = {}) {
  const parts = [];
  for (const d of (sz.drugs || []).slice(0, 4)) if (d.substance) parts.push(`${num(d.kg) > 0 ? weight(num(d.kg)) : num(d.qty) > 0 ? `${num(d.qty).toLocaleString('en-US')} ${str(d.unit || 'units', 12)}${num(d.qty) === 1 ? '' : 's'}` : ''} ${str(d.substance, 30)}`.trim());
...
export function compactCluster(c, currentCut) {
  ...
  return {
    ...
    location: loc ? {
      ...
      locality: loc.locality ? str(loc.locality, 80) : null,
      source: loc.source ? str(loc.source, 12) : 'rules',
      evidence: Array.isArray(loc.evidence) ? loc.evidence.slice(0, 2).map(e => str(e, 300)) : [],
    } : null,
