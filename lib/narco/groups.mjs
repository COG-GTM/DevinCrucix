...
// "the Sinaloa and Jalisco New Generation cartels": a coordinated list of group names sharing one
// "cartels" head noun; none of the names are places.
const CAP_WORD = '[A-Z\\u00C0-\\u00DE][\\w\\u00C0-\\u017F]+';
const CAP_PHRASE = `${CAP_WORD}(?:\\s+${CAP_WORD}){0,3}`;
const LIST_RE = new RegExp(`\\b${CAP_PHRASE}(?:\\s*(?:,|and|y|or|e)\\s+${CAP_PHRASE})+\\s+(?:cartels|c[a\\u00e1]rteles)\\b`, 'g');
const LIST_SEP_RE = /\s*(?:,|\band\b|\by\b|\bor\b|\be\b)\s+/;
export function maskGroupNames(text, idx = loadGroups()) {
  return String(text || '').replace(LIST_RE, m => ' '.repeat(m.length)).replace(idx.maskRe, m => ' '.repeat(m.length));
}
// Distribute the shared head noun so each listed group is matchable: "Sinaloa cartel and Jalisco New Generation cartel".
function expandGroupLists(text) {
  return String(text || '').replace(LIST_RE, m => m.replace(/\s+(?:cartels|c[a\u00e1]rteles)$/i, '').split(LIST_SEP_RE).map(n => `${n} cartel`).join(' and '));
}
...
export function findGroups(text, idx = loadGroups()) {
  const t = fold(expandGroupLists(text));
  const hits = new Map();
