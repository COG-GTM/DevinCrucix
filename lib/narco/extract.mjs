...
export function extractSeizures(text) {
  ...
  while ((m = CASH_RE.exec(s)) !== null) {
    ...
    if (CASH_NOT_RE.test(s.slice(m.index + m[0].length, m.index + m[0].length + 40)) || CASH_VALUE_RE.test(fold(s.slice(Math.max(0, m.index - 40), m.index))) || !CASH_CTX_RE.test(sentenceAt(s, m.index))) continue;
