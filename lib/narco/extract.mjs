...
export const EVENT_TYPES = [
  ...
  { id: 'homicide', label: 'Homicide / execution', re: /\b(?:kill(?:s|ed|ing)?|murder(?:s|ed)?|slain|execut(?:es|ed|ion)|shot (?:dead|to death|and killed)|fatally shot|die[ds] after being shot|died (?:of|from) (?:a |his |her |their )?(?:gunshot )?wounds?|gunned down|found dead|dismembered|beheaded|decapitad|asesinad[oa]s?|ejecutad[oa]s?|homicid|dead bodies?|(?:\d+|several|multiple|dozens? of|dismembered|burned) bodies|bodies (?:were |was )?(?:found|dumped|discovered|recovered|left)|body (?:was )?found|left \w+ dead)\b/i },
...
const NUM_RE_SRC = `\\b(\\d{1,3}(?:[,.]\\d{3})*(?:\\.\\d+)?|${Object.keys(NUM_WORDS).sort((a, b) => b.length - a.length).join('|')})`;
...
const COUNT_PATTERNS = {
  killed: [
    new RegExp(`${NUM_RE_SRC}\\s+(?:people|persons|men|women|civilians|victims|migrants|gunmen|suspects|members|sicarios|police(?: officers)?|officers|soldiers|students|bodies|corpses)?\\s*(?:were |was |had been |found |left )?(?:killed|dead|murdered|executed|slain|shot dead|shot to death|gunned down|massacred|found dead)\\b`, 'gi'),
    ...
    new RegExp(`${NUM_RE_SRC}\\s+(?:bodies|corpses|cuerpos|cad[aá]veres|muertos|asesinados|ejecutados|fallecidos)\\b`, 'gi'),
  ...
  kidnapped: [
    new RegExp(`${NUM_RE_SRC}\\s+(?:people|persons|men|women|migrants|students|workers|miners|farmers|others)?\\s*(?:were |was |have been |had been |remain |are )?(?:kidnapped|abducted|missing|disappeared|taken(?!\\s+(?:to|into|in|for)\\b)|secuestrados|desaparecidos|levantados)\\b`, 'gi'),
...
// A lone named victim ("a man ... died after being shot") is one killed when no numeral says otherwise.
const SINGLE_KILLED_RE = /\b(?:a|an|one|the)\s+(?:young |elderly |unidentified )?(?:man|woman|person|boy|girl|teenager|minor|youth|police officer|officer|soldier|driver|businessman|journalist|activist|lawyer|mayor|candidate|priest)\b[^.;]{0,80}?\b(?:was (?:killed|murdered|executed|shot dead|shot to death|gunned down|found dead)|die[ds] after being shot|was shot and killed|shot and killed)\b|\b(?:man|woman|officer|journalist|mayor|candidate|priest)\s+(?:dies|killed|murdered|executed|shot dead|gunned down|found dead)\b/i;
...
export function extractCounts(text) {
  ...
  if (out.killed == null && SINGLE_KILLED_RE.test(s)) out.killed = 1;
  return out;
}
...
const PILLS_RE = /(\d{1,3}(?:,\d{3})*|\d+(?:\.\d+)?\s*million)\s+(?:fentanyl[- ]laced\s+|counterfeit\s+|fake\s+)?(?:fentanyl\s+)?(pills|tablets|pastillas)\b/gi;
...
const VEHICLES_RE = new RegExp(`${NOT_MONEY}${NUM_RE_SRC}\\s+(?:armored |stolen |tactical |monster )?(?:vehicles?|trucks?|cars?|veh[ií]culos?|camionetas?)\\b`, 'gi');
...
// "...with an estimated (street) value of $360 million" prices the drugs; it is not cash.
const CASH_VALUE_RE = /(?:valued?|worth|estimated|priced|price|valor(?:ados?)?)\s*(?:street value\s*)?(?:of|at|en|de)?\s*(?:approximately|about|over|more than|nearly|around|roughly|some|up to|aproximadamente|mas de|unos)?\s*$/i;
// Programme-wide totals ("30,000 firearms confiscated during the administration") are not this incident's haul.
const CUMULATIVE_RE = /\b(?:during (?:the |this |president |presidenta? )?[\w.'\u2019 ]{0,40}?(?:administration|sexenio|term|presidency|government)|since (?:19|20)\d\d|since (?:taking|he took|she took) office|so far this year|this year alone|over the (?:past|last)|in the (?:past|last) (?:\w+ )?(?:years?|months?|decades?)|a year\b|annually|per year|each year|every year|to date|nationwide|across the country|en lo que va|durante (?:el|la|su) (?:sexenio|administraci[o\u00f3]n|gobierno))\b/i;
...
export function extractSeizures(text) {
  ...
  while ((m = QTY_RE.exec(s)) !== null) {
    ...
    const sent = sentenceAt(s, m.index);
    if (qty == null || qty <= 0 || !DRUG_CTX_RE.test(sent) || CUMULATIVE_RE.test(sent)) continue;
  ...
  while ((m = PILLS_RE.exec(s)) !== null) {
    ...
    const sent = sentenceAt(s, m.index);
    if (!qty || drugs.some(d => d.unit === 'pill' && d.qty === qty) || !DRUG_CTX_RE.test(sent) || CUMULATIVE_RE.test(sent)) continue;
  ...
  while ((m = WEAPONS_RE.exec(s)) !== null) {
    const n = parseNumber(m[1]);
    const sent = sentenceAt(s, m.index);
    if (n && n <= 100000 && SEIZE_CTX_RE.test(sent) && !CUMULATIVE_RE.test(sent) && (weapons == null || n > weapons)) weapons = n;
  }
  let vehicles = null;
  ...
  while ((m = VEHICLES_RE.exec(s)) !== null) {
    const n = parseNumber(m[1]);
    const sent = sentenceAt(s, m.index);
    if (n && n <= 10000 && SEIZE_CTX_RE.test(sent) && !CUMULATIVE_RE.test(sent) && (vehicles == null || n > vehicles)) vehicles = n;
  }
  const cash = [];
  CASH_RE.lastIndex = 0;
  while ((m = CASH_RE.exec(s)) !== null) {
    ...
    const sent = sentenceAt(s, m.index);
    if (CASH_NOT_RE.test(s.slice(m.index + m[0].length, m.index + m[0].length + 40)) || CASH_VALUE_RE.test(fold(s.slice(Math.max(0, m.index - 40), m.index))) || !CASH_CTX_RE.test(sent) || CUMULATIVE_RE.test(sent)) continue;
