...
const QTY_RE = new RegExp(`(\\d{1,3}(?:,\\d{3})*(?:\\.\\d+)?|\\d+(?:\\.\\d+)?)\\s*(?:million\\s+)?(kilograms?|kilos?|kg|pounds?|lbs?|tons?|tonnes?|toneladas?|kilogramos?|libras|grams?|gramos|liters?|litros|gallons?|doses|dosis|pills|tablets|pastillas)\\s+(?:of\\s+|de\\s+)?([A-Za-z\\u00C0-\\u017F ]{3,40}?)(?=[,.;)]|$|\\s+(?:and|with|worth|valued|hidden|were|was|in|that|estimated)\\b)`, 'gi');
const PILLS_RE = /(\d{1,3}(?:,\d{3})*|\d+(?:\.\d+)?\s*million)\s+(?:fentanyl[- ]laced\s+|counterfeit\s+|fake\s+)?(?:fentanyl\s+)?(pills|tablets|pastillas)/gi;
const WEAPONS_RE = new RegExp(`${NUM_RE_SRC}\\s+(?:(?:assault |long |high-powered |automatic )?(?:rifles?|weapons?|firearms?|guns?|handguns?|pistols?|grenades?|magazines?|armas(?: largas| de fuego)?|fusiles|granadas|cargadores))`, 'gi');
...
const VEHICLES_RE = new RegExp(`${NUM_RE_SRC}\\s+(?:armored |stolen |tactical |monster )?(?:vehicles?|trucks?|cars?|veh[ií]culos|camionetas)`, 'gi');
...
export function extractSeizures(text) {
  ...
  while ((m = QTY_RE.exec(s)) !== null) {
    ...
    drugs.push({ substance, qty: q, unit: fold(m[2]).replace(/s$/, ''), ...(kg != null ? { kg: Number(kg.toFixed(2)) } : {}) });
  }
  PILLS_RE.lastIndex = 0;
  while ((m = PILLS_RE.exec(s)) !== null) {
    const raw = m[1];
    const qty = /million/i.test(raw) ? Number(raw.replace(/[^\d.]/g, '')) * 1e6 : parseNumber(raw);
    if (!qty || drugs.some(d => d.unit === 'pill' && d.qty === qty)) continue;
  ...
  while ((m = WEAPONS_RE.exec(s)) !== null) {
    const n = parseNumber(m[1]);
    if (n && n <= 100000 && (weapons == null || n > weapons)) weapons = n;
  }
  let vehicles = null;
  VEHICLES_RE.lastIndex = 0;
  while ((m = VEHICLES_RE.exec(s)) !== null) {
    const n = parseNumber(m[1]);
    if (n && n <= 10000 && (vehicles == null || n > vehicles)) vehicles = n;
  }
  const cash = [];
  CASH_RE.lastIndex = 0;
