        ...
        with db.transaction() as cur:
            cur.execute(
                """INSERT OR IGNORE INTO sources
                   ...
                   VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                (
                    src["slug"], src["outlet_name"], src["feed_url"], src["feed_type"], src["language"],
                    src["country_of_publication"], src["region_tag"], src["reliability"], src["source_type"],
                    src["discovery_date"], src["discovery_method"], src["terms_url"], src["notes"], src["phase"],
                    src["enabled"], src["content_policy"], now, now,
                ),
            )
...
def upsert_source(db: Database, src: dict[str, Any]) -> dict[str, Any]:
    ...
    with db.transaction() as cur:
        cur.execute(
            ...
            (
                clean["slug"], clean["outlet_name"], clean["feed_url"], clean["feed_type"], clean["language"],
                clean["country_of_publication"], clean["region_tag"], clean["reliability"], clean["source_type"],
                clean["discovery_date"], clean["discovery_method"], clean["terms_url"], clean["notes"],
                clean["phase"], clean["enabled"], clean["content_policy"], now, now,
            ),
        )
